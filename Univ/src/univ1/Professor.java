...
//		ImageIcon cImageIcon = new ImageIcon(cImg);
		...
		sBox.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				case "교수코드":
					sQuary = "select * from professor where pCode = ?";
					break;
				case "교수명":
					sQuary = "select * from professor where name = ?";
					break;
				case "학과명":
					sQuary = "select * from professor where dept = ?";
					break;
				case "성별":
...
//					table.clearSelection();
		...
		inputBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int result = insert(pCodeField.getText(), nameField.getText(), adrField.getText(),
						...
						dept, officeField.getText(), gender);
				
				if (result != 0) {
					JOptionPane.showMessageDialog(null, pCodeField.getText()+"의 정보가 입력되었습니다.");
					getListAll();
					reset();
				}
			}
		...
		editBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (result != 0) {
					JOptionPane.showMessageDialog(null, pCode+"의 정보를 수정했습니다.");
					getListAll();
					reset();
					selectRow = -1;
				}
		...
		delBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int i = JOptionPane.showConfirmDialog(null, pCode+"의 정보를 삭제하시겠습니까?", "삭제", JOptionPane.YES_NO_OPTION);
				
				if (i == 0) {
					int result = delete(pCode);
					
					if (result != 0) {
						JOptionPane.showMessageDialog(null, pCode+"의 정보를 삭제했습니다.");
						getListAll();
						reset();
						selectRow = -1;
					}
	...
	public void getListAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				String[] data = {pCode,name,addr,rNum,phone,tel,year,degree,dept,office,gender};
	...
	public int update(String pCode, String name, String addr, String rNum, String phone, String tel,
		...
		String quary = "update professor set name = ?, addr = ?, rNum = ?, phone = ?, tel = ?, year = ?,"
				+ "degree = ?, dept = ?, office = ?, gender = ? where pCode = ?";
