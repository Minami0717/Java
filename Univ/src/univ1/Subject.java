...
//		ImageIcon cImageIcon = new ImageIcon(cImg);
		...
		sBox.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				case "code":
					sQuary = "select * from subject where subjectNum = ?";
					break;
				case "subject":
					sQuary = "select * from subject where subjectName = ?";
					break;
				case "dept_code":
					sQuary = "select * from subject where deptName = ?";
					break;
				case "prof_code":
					sQuary = "select * from subject where profName = ?";
					break;
				case "year":
					sQuary = "select * from subject where year = ?";
					break;
				default:
					break;
				}
...
//					table.clearSelection();
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (dtm.getRowCount() != 0) {
					for (int i = 0; i < table.getRowCount(); i++) {
						if (sCodeField.getText().equals(table.getValueAt(i, 0))) {
							JOptionPane.showMessageDialog(null, "같은 교과목코드가 있습니다.");
							return;
						}
					}
				}
				...
				int result = insert(sCodeField.getText(), sNameField.getText(), d, y, g, t,
						hourField.getText(), p, creditField.getText());
		...
		editBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (dtm.getRowCount() != 0) {
					for (int i = 0; i < table.getRowCount(); i++) {
						if (i != selectRow && sCodeField.getText().equals(table.getValueAt(i, 0))) {
							JOptionPane.showMessageDialog(null, "같은 교과목코드가 있습니다.");
							return;
						}
					}
				...
				int result = update(sCode, sNameField.getText(), d, y, g, t,
						hourField.getText(), p, creditField.getText());
	...
	public void getListAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				String profName = rs.getString(8);
				String credit = rs.getString(9);
				String[] data = {subjectNum,subjectName,deptName,year,grade,term,hour,profName,credit};
	...
	public int update(String sCode, String sName, String dName,String year,String grade,String term,
			String hour,String profName,String credit) {
		int result = 0;
		String quary = "update subject set subjectName = ?, deptName = ?, year = ?, grade = ?,"
				+ "term = ?, hour = ?, profName = ?, credit = ? where subjectNum = ?";
	...
	public int delete(String sCode) {
		int result = 0;
		String quary = "delete from subject where subjectNum = ?";
