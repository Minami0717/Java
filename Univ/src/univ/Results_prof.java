...
public class Results_prof extends JFrame {
	...
	JComboBox yBox, gBox, tBox, sBox, cBox;
...
//		ImageIcon cImageIcon = new ImageIcon(cImg);
		...
		String[] c = {"A+","A","A-","B+","B","B-","C+","C","C-","D+","D","D-","F"};
		...
		tc.setCellEditor(new DefaultCellEditor(cBox));
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				sum = attended + report + middle + finals + added;
				int sum1 = attended + report + middle + finals;
				...
				if (sum > 100) {
					...
					if (sum1 == 100)
						table.setValueAt(0, selectRow, 6);
					return;
				}
				
				table.setValueAt(sum, selectRow, 7);
				table.setValueAt(grade, selectRow, 8);
			}
		});
		...
		saveBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < table.getRowCount(); i++) {
					String stdCode = table.getValueAt(i, 0).toString();
					int attended = Integer.parseInt(table.getValueAt(i, 2).toString());
					int report = Integer.parseInt(table.getValueAt(i, 3).toString());
					int middle = Integer.parseInt(table.getValueAt(i, 4).toString());
					int finals = Integer.parseInt(table.getValueAt(i, 5).toString());
					int added = Integer.parseInt(table.getValueAt(i, 6).toString());
					int sum = Integer.parseInt(table.getValueAt(i, 7).toString());
					String grade = table.getValueAt(i, 8).toString();
					result = update(attended, report, middle, finals, added, sum, grade, stdCode);
				}
	...
	public void addSub() {
		...
		String quary = "select subject from subject where prof_code = ?";
	...
	public String getStdName(String stdCode) {
		String quary = "select name from student where sNum = ?";
		String stdName = "";
		try {
			pstmt = univMenu.connector.conn.prepareStatement(quary);
...
//	}
	...
	public int update(int attended, int report, int middle, int finals, int added, int sum,
		...
		String quary = "update results set attended = ?, report = ?, middle = ?, final = ?, "
				+ "added = ?, sum = ?, grade_value = ? where std_code = ? and prof_code = ?";
		...
		try {
			...
			pstmt.setString(9, pCodeField.getText());
...
//	}
	...
	public void search() {
		...
		String quary = "select std_code, attended, report, middle, final, added,"
				+ "sum, grade_value from results where subj_code = ? and prof_code = ?";
		
		try {
			pstmt = univMenu.connector.conn.prepareStatement(quary);
			pstmt.setString(1, searchField.getText());
			pstmt.setString(2, pCodeField.getText());
			rs = pstmt.executeQuery();
			while (rs.next()) {
				String std_code = rs.getString(1);
				...
				String[] data = {std_code,getStdName(std_code),attended,report,middle,fainal,added,sum,grade_value};
