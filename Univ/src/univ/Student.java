...
//					table.clearSelection();
		...
		inputBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (result != 0) {
					JOptionPane.showMessageDialog(null, sNumField.getText()+"의 학생 정보가 입력되었습니다.");
					getListAll();
					reset();
				}
			}
		...
		editBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (result != 0) {
					JOptionPane.showMessageDialog(null, sCode+"의 학생 정보를 수정했습니다.");
					getListAll();
					reset();
					selectRow = -1;
				}
		...
		delBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int i = JOptionPane.showConfirmDialog(null, sCode+"의 학생 정보를 삭제하시겠습니까?", "삭제", JOptionPane.YES_NO_OPTION);
				
				if (i == 0) {
					int result = delete(sCode);
					
					if (result != 0) {
						JOptionPane.showMessageDialog(null, sCode+"의 학생 정보를 삭제했습니다.");
						getListAll();
						reset();
						selectRow = -1;
					}
