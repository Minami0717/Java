...
public class DB_K_Edit extends JFrame {
	ResultSet rs;
	...
	DB_K_Edit(DB_K_Menu k_Menu) {
		...
		titleLabel = new JLabel("K_League_Player 선수 수정");
		...
		edit.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				int result = update(nameField.getText(), ageField.getText(), weightField.getText(),
						posField.getText(), bNumField.getText(), homeField.getText(), idx);
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				idx = Integer.parseInt(table.getValueAt(table.getSelectedRow(), 0).toString());
				nameField.setText(table.getValueAt(srow, 1).toString());
				ageField.setText(table.getValueAt(srow, 2).toString());
				weightField.setText(table.getValueAt(srow, 3).toString());
				posField.setText(table.getValueAt(srow, 4).toString());
				bNumField.setText(table.getValueAt(srow, 5).toString());
				homeField.setText(table.getValueAt(srow, 6).toString());
			}
		});
	...
	public int update(String name, String age, String weight, String position, String backNumber, String hometown, int idx) {
		int result = 0;
		String quary = "UPDATE player SET name = ?, age = ?, weight = ?, position = ?, backNumber = ?,"
				+ "hometown = ? WHERE idx = ?";
		...
		JOptionPane.showMessageDialog(this, name+"선수가 수정되었습니다.");
		return result;
	}
